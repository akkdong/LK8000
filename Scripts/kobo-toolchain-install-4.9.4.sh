...
[ -z "$TC"] && TC=arm-linux-gnueabihf
[ -z "$BUILD_DIR" ] && BUILD_DIR=$HOME/kobo/tmp-Kobo-4.9.4
[ -z "$TARGET_DIR" ] && TARGET_DIR=$HOME/kobo/arm-linux-gnueabihf-4.9.4
...
cd zlib-1.2.11
CC=$TC-gcc$TC_SUFFIX CFLAGS="-O3 -march=armv7-a -mfpu=neon -ftree-vectorize -mvectorize-with-neon-quad -ffast-math -funsafe-math-optimizations -funsafe-loop-optimizations" \
...
cd boost_1_66_0
./bootstrap.sh
echo "using gcc : arm : $TC-g++$TC_SUFFIX : cxxflags=-O3 -march=armv7-a -mfpu=neon -ftree-vectorize -mvectorize-with-neon-quad -ffast-math -funsafe-math-optimizations -funsafe-loop-optimizations ;" > user-config.jam
sudo PATH=$USER_PATH:$PATH \
    ./bjam toolset=gcc \
           variant=release \
           link=shared \
